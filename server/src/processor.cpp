...
static constexpr const char ARCHIVES_ROOT[] = "htdocs/deps/time_walk_images/";
...
static constexpr const char FN_IMAGE[] = "image";
...
{
	log(Log::INFO, "Processing Request: ", request);
...
void get_response(std::string & response, const std::string & request, Log & log)
{
	...
	case Command::LIST_POPULAR:
	{
		...
		std::string first_part = std::to_string(static_cast<size_t>(Response::VALID_REQUEST));
		first_part.push_back(SEPERATOR);
		response = first_part.append(response); 
		response.pop_back(); // remove last seperator
	}
		break;
	case Command::GET_THEMES:
	{
		...
		else
		{
			...
			path.append(region);
			path.append(theme);
			log(Log::INFO, "Getting Theme ", theme, " of ", region);
			get_file(response, path, log);
			response.pop_back(); // remove last \n
		}
		break;
	}
	case Command::GET_REGION_GPS:
        {
                std::string region;
                if (get_from_stream(region, stream))
                {
                ...
                else
                {
                        ...
                        path.append(region);
                        path.append(FN_GPS);
                        log(Log::INFO, "Getting GPS of ", region);
                        get_file(response, path, log);
			response.pop_back(); // remove last \n
                }
                break;
        }
	case Command::GET_LANDMARK_GPS:
	{
		std::string region;
		std::string landmark;
		if (get_from_stream(region, stream) ||
	...
	case Command::LIST_THEMES:
	{
		...
		else
		{
			...
			log(Log::INFO, "Scanning Directory: ", path.string());
			...
			else
			{
				...
				for (auto && item : fs::directory_iterator(path))
					if (!fs::is_directory(item.path()) &&
					    fs::is_regular_file(item.path()) &&
					    item.path().filename().string() != FN_GPS &&
					    item.path().filename().string() != FN_MAIN_PHOTO)
					{
...
{
	log(Log::INFO, "Loading File: ", path.string());
...
void get_image(std::string & response, const fs::path & path, Log & log)
{
	...
	log(Log::INFO, "Validating Image: " + validate.string());
